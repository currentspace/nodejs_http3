...
describe('browser compatibility smoke', { skip: !ENABLED && 'set HTTP3_BROWSER_E2E=1 to enable browser tests' }, () => {
  let port = 0;
    ...
    const server = serveFetch({
      ...
      fetch: async (req: Request): Promise<Response> => {
        ...
        return new Response(
          ...
          { headers: { 'content-type': 'text/html; charset=utf-8', 'alt-svc': `h3=":${String(port)}"; ma=300` } },
        );
      },
    });
  ...
  async function runBrowserSmoke(kind: 'chromium' | 'firefox'): Promise<void> {
    ...
    try {
      ...
      assert.ok(
        protocol === '' || protocol === 'h2' || protocol.startsWith('h3'),
        `expected browser to report h2/h3 or leave nextHopProtocol empty, got ${protocol || '<empty>'}`,
      );
      await context.close();
    } finally {
      await browser.close();
  ...
  it('works in Chromium', async () => {
    await runBrowserSmoke('chromium');
  });

  it('works in Firefox', async () => {
    await runBrowserSmoke('firefox');
  });
});
